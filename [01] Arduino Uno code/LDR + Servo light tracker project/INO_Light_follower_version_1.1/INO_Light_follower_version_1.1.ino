...
//This version of the conditions check whether both of the ldrs supports to change servo position. 
  ...
  if(ldr1 - ldr4 >= difference && ldr2 - ldr3 >= difference && yAngle <=170)
  {
    yAngle+=change; 
    servoY.write(yAngle); 
    delay(wait); 
  }
  else if(ldr4 - ldr1 >= difference && ldr3 - ldr2 >= difference &&  yAngle >= 10) 
  {
    yAngle-=change; 
    servoY.write(yAngle); 
    delay(wait); 
  ...
  if(ldr1 - ldr2 >= difference && ldr4 - ldr3 >= difference && xAngle >= 0)
  {
    xAngle-=change; 
    servoX.write(xAngle); 
    delay(wait); 
  }
  else if(ldr2 - ldr1 >= difference && ldr3 - ldr4 >= difference && xAngle <= 180)
  {
    xAngle+=change; 
    servoX.write(xAngle); 
    delay(wait); 
